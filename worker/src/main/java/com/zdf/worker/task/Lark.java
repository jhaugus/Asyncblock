...
public class Lark implements AsyncExecutable {
    ...
    public TaskRet printMsg2(String msg) {
        ...
        AsyncTaskSetStage asyncTaskSetStage = null;
        try {
            Method method = this.getClass().getMethod("printMsg3", String.class);
            asyncTaskSetStage = setStage(this.getClass(), method.getName(), new Object[]{"我要开花22！"}, method.getParameterTypes());
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        }
        return new TaskRet("执行成功",asyncTaskSetStage);
    }

    public TaskRet printMsg3(String msg) {
        System.out.println("第3阶段开启中文打印: " + msg);
